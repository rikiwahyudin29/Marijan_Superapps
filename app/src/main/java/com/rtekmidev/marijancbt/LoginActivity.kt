...
class LoginActivity : AppCompatActivity() {
    ...
    private lateinit var otpLauncher: androidx.activity.result.ActivityResultLauncher<Intent>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        otpLauncher = registerForActivityResult(androidx.activity.result.contract.ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == RESULT_OK) {
                ...
                if (!otpCode.isNullOrEmpty()) {
                    ...
                    performLogin(pendingUsername, pendingPassword, pendingDeviceId, pendingDeviceName, otpCode, progressBar, btnLogin)
                }
            }
        }
        ...
        ivTogglePassword.setOnClickListener {
            ...
            if (isPasswordVisible) {
                ...
                ivTogglePassword.setColorFilter("#38BDF8".toColorInt()) // Highlight color
            } else {
                // Hide Password
                etPassword.inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_PASSWORD
                ivTogglePassword.setColorFilter("#9CA3AF".toColorInt()) // Default color
            }
        ...
        btnLogin.setOnClickListener {
            ...
            performLogin(username, password, deviceId, deviceName, null, progressBar, btnLogin)
        }
    }

    private fun performLogin(username: String, password: String, deviceId: String, deviceName: String, otpCode: String?, progressBar: ProgressBar, btnLogin: Button) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = ApiClient.instance.login(username, password, deviceId, deviceName, otpCode)
                ...
                withContext(Dispatchers.Main) {
                    ...
                    val isRequires2FA = statusElement != null && statusElement.isJsonPrimitive && statusElement.asString == "requires_2fa"

                    if (response.isSuccessful && isRequires2FA) {
                    ...
                    } else if (response.isSuccessful && isStatusTrue) {
