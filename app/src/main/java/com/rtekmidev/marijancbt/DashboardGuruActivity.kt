...
class DashboardGuruActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Force intent ROLE to GURU so PresensiFragment can read it correctly
        ...
        // Set token ke ApiClient biar selalu terkirim
        ...
        fun updateNavSelection(position: Int) {
            val unselectedColor = if (isNightMode) android.graphics.Color.parseColor("#B3FFFFFF") else android.graphics.Color.parseColor("#9CA3AF")
            val selectedColor = if (isNightMode) android.graphics.Color.parseColor("#FFFFFF") else android.graphics.Color.parseColor("#1E3A8A")
    ...
    private fun bukaScannerQR() {
        ...
        options.setPrompt("Arahkan ke QR Code Kehadiran (Guru)")
    ...
    private fun prosesAbsenQR(qrToken: String) {
        val sharedPref = getSharedPreferences("SesiGuru", Context.MODE_PRIVATE)
        var identifier = sharedPref.getString("id_user", "") ?: ""
        if (identifier.isEmpty()) {
            identifier = sharedPref.getString("username", "") ?: ""
        }
                            ...
                            try {
                                ...
                                if (!errorStr.isNullOrEmpty()) {
                                    if (errorStr.startsWith("{")) {
                                        ...
                                        // Tampilkan sebagian response asli (HTML / Plain text)
        ...
        override fun createFragment(position: Int): Fragment {
            return when (position) {
                ...
                1 -> {
                    ...
                    val bundle = Bundle()
                    bundle.putString("ROLE", "GURU")
                    fragment.arguments = bundle
                    fragment
                }
                2 -> AkademikGuruFragment()
                3 -> PlaceholderFragment()
