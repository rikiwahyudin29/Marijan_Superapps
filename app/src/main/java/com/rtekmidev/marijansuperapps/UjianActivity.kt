...
import com.rtekmidev.marijansuperapps.api.Soal
import com.rtekmidev.marijansuperapps.api.SubmitRequest
...
class UjianActivity : AppCompatActivity() {
    ...
    private fun muatDataOffline() {
        ...
        if (jsonString.isNotEmpty()) {
            ...
            if (backupJson != null) {
                try {
                    val typeJawaban = object : TypeToken<List<com.rtekmidev.marijansuperapps.api.JawabanSiswa>>() {}.type
                    val backupAnswers: List<com.rtekmidev.marijansuperapps.api.JawabanSiswa> = Gson().fromJson(backupJson, typeJawaban)
    ...
    private fun renderBenarSalah(wadah: LinearLayout, soal: Soal, jawaban: String) {
        ...
        for (opsi in soal.opsi) {
            ...
            val rbBenar = RadioButton(this).apply {
                text = "Benar"
                buttonTintList = ColorStateList.valueOf(Color.parseColor("#059669"))
