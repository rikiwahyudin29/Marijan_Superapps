...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun getAllJawaban(): List<com.rtekmidev.marijansuperapps.api.JawabanSiswa> {
        val db = this.readableDatabase
        val cursor = db.rawQuery("SELECT $COL_SOAL_ID, $COL_JENIS_SOAL, $COL_JAWABAN_TEKS FROM $TABLE_JAWABAN", null)
        val list = mutableListOf<com.rtekmidev.marijansuperapps.api.JawabanSiswa>()
        if (cursor.moveToFirst()) {
            do {
                list.add(com.rtekmidev.marijansuperapps.api.JawabanSiswa(
